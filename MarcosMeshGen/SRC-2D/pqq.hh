//-----------------------------------------------------------------------------------------
void build_dual_mesh_with_boundary_quads( mesh_2Dv & mesh, mesh_2Dv & dual_mesh ) {
  ...
  for ( int iV=0 ; iV<mesh.n_vertex() ; ++iV ) {
	...
	next_iR = iR1 ;
      ...
      // iF is set from previous if-block
  ...
  } // end of -->   for ( int iV=0 ; iV<mesh.n_vertex() ; ++iV ) {

  // int nR_debug = regn_vlist.size() ; TBR

  for ( int ilV=0 ; ilV<mesh.n_bvertex() ; ++ilV ) {
    int iV = mesh.get_bnd_vrtx(ilV) ;
    ...
    if ( nVF!=4 ) {
      ...
      // ----------------------------------
      // the dual cell is not a hexagon!
      // ----------------------------------
	...
	regn_vlist.push_back( face_idx[iF] ) ;
      ...
      // ----------------------------------
      // hexagons are split into two quads
      // ----------------------------------
      ...
      next_iR = iR0 ;
      search_for_next_regn( iF, next_iR, mesh, flist, mask_F ) ;
      regn_vlist.push_back( next_iR ) ;
