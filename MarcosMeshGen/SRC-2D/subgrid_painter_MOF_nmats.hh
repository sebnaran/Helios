...
// .....
// this routine processes (max) nmat=2 different materials per cell
// nmat is a static constant with assigned value == 2
// .....
...
public:
  sub_mesh2Dv_painter_MOF( mesh_2Dv & _mesh, int _nintf=1 ) : 
    mesh(_mesh), epsi(1e-12), nx(100), ny(100), icrv(-1), nintf(_nintf)
  {
    setup() ;
  }
  ~sub_mesh2Dv_painter_MOF() {}

  // reset number of interface
  void set_nintf( int _nintf ) {
    nintf = _nintf ;
  }
...
void sub_mesh2Dv_painter_MOF::paint_multiple_lines( vector<double> & _x0_vec, vector<double> & _y0_vec, 
  ...
  assert( nintf == _x0_vec.size() ) ;
  for ( int k=0; k<nintf; ++k ) {
    x0_vec.push_back( _x0_vec[k] ) ;
    y0_vec.push_back( _y0_vec[k] ) ;
    x1_vec.push_back( _x1_vec[k] ) ;
    y1_vec.push_back( _y1_vec[k] ) ;
  }
...
int sub_mesh2Dv_painter_MOF::mlines_izone( double xV, double yV ) {
  ...
  assert( icrv==10 ) ;
  int retval = nintf ; // undef  zone, probable intersection
  for ( int k=0; k<nintf; ++k ) {
    double ri = interface_line( k, xV, yV ) ;
    if ( ri<0. ) { retval = k ; break ; }
  }
  retval = retval%nmat ; 
  //MSGF("end of sub_mesh2Dv_painter_MOF::mlines_izone") ;
  return retval ;
...
double sub_mesh2Dv_painter_MOF::interface_line( int k, double x, double y ) {
  assert( 0<=k && k<x0_vec.size() ) ;
  double x0 = x0_vec[k] ;
  double y0 = y0_vec[k] ;
  double x1 = x1_vec[k] ;
  double y1 = y1_vec[k] ;
  double rv = ( x-x0 )*( y1-y ) - ( x1-x )*( y-y0 ) ;
  //MSG("interface_line -->> ") ; VAL(x) ; VAL(y) ; PRT(rv) ; // TBR
  return rv ;
}
