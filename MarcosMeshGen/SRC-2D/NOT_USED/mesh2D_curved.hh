...
public:
  ...
  // number of nodes on face iF
  // must be at least 2
  int n_face_node( int iF ) {
    assert( 0<=iF && iF<cvd_flist.size() ) ;
    int n_face_nodes =  cvd_flist[iF].n_node() ;
    assert( n_face_nodes>=2 ) ;
    return n_face_nodes ;
  }
  ...
  double face_dist( double t,  int iF ) ;
  ...
  // moments of the tangent vector X'(t) of the parameterized curve X(t)
  void face_rotated_moments( int iF, double & zmx, double & zmy, double & fmx, double & fmy ) ;
  void face_moment( double t, int m, int iF, double & mx, double & my ) ;
...
double mesh_2Dc :: face_dist( double t, int iF ) {
  //assert( 0<=t    && t<=1        ) ;
  assert( 0<=iF   && iF<n_face() ) ;
  assert( is_curved_face(iF)     ) ;
  double retval =0. ;
...
// integral of face's rotated moments contain face lengths
// (mx0,my0): integral of ROT (1-t) X'(t)
// (mx1,my1): integral of ROT t     X'(t)
// ROT = (  0 1 )
//       ( -1 0 )
// (rotation matrix)
void mesh_2Dc :: face_rotated_moments( int iF, 
  ...
  double tx[3] = { face_derv(t0,iF,0), face_derv(t1,iF,0), face_derv(tm,iF,0) } ; 
  double ty[3] = { face_derv(t0,iF,1), face_derv(t1,iF,1), face_derv(tm,iF,1) } ; 

  my0 = -( (1.-t0)*tx[0] + 4.*(1.-tm)*tx[2] + (1.-t1)*tx[1] )/6. ;
  mx0 = +( (1.-t0)*ty[0] + 4.*(1.-tm)*ty[2] + (1.-t1)*ty[1] )/6. ;
  
  my1 = -( t0*tx[0] + 4.*tm*tx[2] + t1*tx[1] )/6. ;
  mx1 = +( t0*ty[0] + 4.*tm*ty[2] + t1*ty[1] )/6. ;
}

// should be eliminated in the final version
// pointwise values of face moment (mx,my) = t^m X'(t)
void mesh_2Dc :: face_moment( double t, int m, int iF, double & mx, double & my ) {
  assert( false ) ;
  //assert( is_curved_face(iF) ) ;
  assert( 0<=t && t<=1. ) ;
  mx = face_derv(t,iF,0) * pow( t, m ) ;
  my = face_derv(t,iF,1) * pow( t, m ) ;
}
...
void mesh_2Dc :: setup_cvd_flist() {
  ...
  for ( int iF=0; iF<nF; ++iF ) {
    ...
    int nFN = 2 ;    
    if ( is_internal_face(iF) ) { // CURVED FACES
	...
	has_bnd_vrtx |= is_boundary_vrtx( iV ) ;
      ...
      if ( !has_tria_neigh || !has_bnd_vrtx ) {
...
void mesh_2Dc :: check_region_geometry( int iR ) { 
  ...
  for ( int ilF=0; ilF<nRF; ++ilF ) {
    int iF = flist[ilF] ;
    if ( is_curved_face(iF) ) {
      int max_np = 2 * n_face_node(iF) ; // 2 is a safe factor
